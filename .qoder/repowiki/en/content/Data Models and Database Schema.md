...
## Update Summary
**Changes Made**
- Comprehensive database schema documentation with complete Prisma model coverage
- Added detailed engineering standards and document type mapping functionality
- Enhanced seeding strategy documentation with standards integration
- Updated architecture diagrams to reflect complete entity relationships
- Added performance considerations for JSONB fields and complex queries
- Expanded caching strategies for standards and questionnaire data
...
## Introduction
This document provides comprehensive data model documentation for the Quiz-to-build database schema. It details all entities, their relationships, fields, data types, constraints, indexes, and referential integrity enforced by the Prisma schema. The schema encompasses organizations, users, questionnaires, sections, questions, visibility rules, and engineering standards with complete seeding functionality for standards integration. It also covers validation rules, business constraints, performance characteristics, caching strategies, and practical usage patterns derived from the application services.

## Project Structure
The data model is defined centrally in the Prisma schema and supported by:
- Prisma client generation and runtime service with query logging
- Redis caching layer for performance optimization
- Seed scripts for initial data and comprehensive engineering standards
- Complete document type and standards mapping system
...
**Diagram sources**
- [prisma.service.ts](file://libs/database/src/prisma.service.ts#L1-L123)
- [redis.service.ts](file://libs/redis/src/redis.service.ts#L1-L96)
- [schema.prisma](file://prisma/schema.prisma#L1-L893)
- [seed.ts](file://prisma/seed.ts#L1-L989)
...
**Section sources**
- [schema.prisma](file://prisma/schema.prisma#L82-L893)

## Architecture Overview
The system follows a comprehensive layered architecture with extensive use of JSONB fields for flexibility and robust standards integration:

- **API services** consume Prisma for database operations and optionally Redis for caching
- **Prisma generates** strongly typed client code with comprehensive schema validation
- **Redis caches** hot data including standards, questionnaire structures, and user sessions
- **PostgreSQL** stores transactional data with JSONB for flexible attributes and robust indexing
- **Standards system** provides comprehensive engineering guidelines mapped to document types
...
- **Standards Integration**: Comprehensive engineering standards system with document type mappings
...
- **JSON schemas**: Options, VisibilityRule conditions, Response values, and progress structures are defined in the schema to guide clients and validation logic
- **Standards validation**: Engineering standards provide comprehensive validation rules for document generation
...
- [standards.service.ts](file://apps/api/src/modules/standards/standards.service.ts#L1-L393)
...
- **Core indexes**:
  ...
  - EngineeringStandards: category, is_active
  - DocumentTypeStandards: document_type_id, standard_id
...
- **Default Organization and Admin User** are created during seeding with comprehensive metadata
- **Main Questionnaire** with 9 sections and 40+ questions covering business foundation, product definition, target market, technology requirements, and security compliance
- **Visibility Rules** demonstrate conditional logic for dynamic questionnaire adaptation
- **Engineering Standards** with comprehensive principles for 7 categories including Modern Architecture, AI-Assisted Development, Coding Standards, Testing QA, Security DevSecOps, Workflow Ops, and Docs Knowledge
- **Document Type Mappings** connect standards to specific document types like Product Architecture, Technology Strategy, and Engineering Handbook

**Section sources**
- [seed.ts](file://prisma/seed.ts#L9-L484)
- [standards.seed.ts](file://prisma/seeds/standards.seed.ts#L18-L208)
- [standards.seed.ts](file://prisma/seeds/standards.seed.ts#L210-L349)
...
- **Find standards for document types** with comprehensive filtering and ordering
- **Query engineering standards** with document type mappings and priority ordering

**Section sources**
- [users.service.ts](file://apps/api/src/modules/users/users.service.ts#L41-L198)
- [session.service.ts](file://apps/api/src/modules/session/session.service.ts#L96-L196)
...
- **Retention policies**:
  ...
  - Engineering standards: versioned with permanent retention
- **Archival process**:
...
- **Redis caching**:
  ...
  - Engineering standards cached with version control
...
- **Standards caching**:
  - Standards categories cached with 1-hour TTL
  - Document type mappings cached with 30-minute TTL
  - Dynamic standards generation results cached
...
- **Initialization**:
  ...
  - UUID and pgcrypto extensions automatically created
...
- **Prisma Service**
  ...
  - Emits slow query logs in development when duration exceeds threshold
...
STANDARDS_SERVICE --> REDIS_SERVICE
```
...
- [session.service.ts](file://apps/api/src/modules/session/session.service.ts#L1-L200)
- [standards.service.ts](file://apps/api/src/modules/standards/standards.service.ts#L1-L200)
...
- **Standards optimization**:
  - Cache frequently accessed standards categories
  - Use selective loading for standards with document mappings
  - Implement lazy loading for large standards collections
- **Caching strategy**:
  ...
  - Engineering standards: Redis TTL 1 hour with version checking
...
- **Slow queries**:
  ...
  - Monitor JSONB query performance with appropriate GIN indexes
- **Database connectivity**:
  - Verify Prisma client initialization and connection lifecycle hooks
  - Check PostgreSQL extensions (UUID, pgcrypto) are available
- **Redis connectivity**:
  - Monitor connection events and error logs; ensure credentials and host/port are configured
  - Verify cache keys are properly formatted for standards and questionnaire data
- **Data inconsistencies**:
  ...
  - Check standards category uniqueness and document type mappings
...
## Conclusion
The Quiz-to-build data model is designed around comprehensive flexibility and scalability, leveraging PostgreSQL's JSONB capabilities and Prisma's strong typing. The schema includes extensive engineering standards integration with document type mappings, comprehensive seeding functionality, and robust caching strategies. The documented relationships, constraints, and patterns enable consistent development and maintenance across modules while supporting complex adaptive questionnaire logic and standards-based document generation.
...
participant Standards as "Engineering Standards"
...
Seeder->>Standards : seedStandards()
Standards->>Prisma : upsert Engineering Standards
Prisma-->>Standards : created/updated
Standards->>Prisma : create DocumentType mappings
Prisma-->>Standards : created/updated
```
...
### Appendix C: Engineering Standards Categories
The system includes comprehensive engineering standards across 7 categories:

1. **Modern Architecture & Design** - Cloud-native foundations, modular design, design before coding
2. **AI-Assisted Development** - AI tools integration, multi-agent systems, human-in-the-loop validation
3. **Coding Standards & Principles** - KISS, YAGNI, DRY, SOLID principles, style guides
4. **Testing & Quality Assurance** - Shift-left testing, layered testing strategy, automated acceptance tests
5. **Security (DevSecOps)** - Security-by-design, automated security scanning, confidential computing
6. **Workflow & Operations** - CI/CD maturity, observability first, FinOps & sustainability
7. **Documentation & Knowledge** - Docs-as-code, document the "why", Architecture Decision Records
