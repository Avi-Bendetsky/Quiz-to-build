  ...
  .action(async (sessionId: string | undefined, options) => {
    ...
    try {
      ...
      suggestions.questions.forEach((q, i) => {
        ...
        console.log(`   ${chalk.gray('Dimension:')} ${q.dimension} | ${chalk.gray('Persona:')} ${q.persona}`);
        console.log(`   ${chalk.gray('Severity:')} ${severityColor(`${(q.severity * 100).toFixed(0)}%`)} | ${chalk.gray('Priority:')} ${priorityIcon}`);
        
        if (q.bestPractice) {
          console.log(`   ${chalk.gray('Best Practice:')} ${chalk.italic(truncate(q.bestPractice, 80))}`);
        }
      });

      // Summary
      console.log('\n' + chalk.gray('─'.repeat(60)));
      console.log(chalk.gray(`Showing ${suggestions.questions.length} of ${suggestions.totalRemaining} remaining questions`));
      
      if (suggestions.topDimension) {
        console.log(chalk.gray(`Focus area: ${suggestions.topDimension.name} (${suggestions.topDimension.remaining} gaps)`));
      }
...
function getSeverityColor(severity: number): chalk.Chalk {
  if (severity >= 0.8) return chalk.red;
  if (severity >= 0.6) return chalk.hex('#FFA500');
  if (severity >= 0.4) return chalk.yellow;
  return chalk.green;
}
