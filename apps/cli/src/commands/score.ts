...
export const scoreCommand = new Command('score')
  ...
  .action(async (sessionId: string | undefined, options) => {
    ...
    try {
      ...
      if (options.detailed && scoreData.dimensions) {
        ...
        const tableData = [
          ...
          ...scoreData.dimensions.map(dim => [
            ...
            dim.score < 0.95 ? chalk.red(`${((0.95 - dim.score) * 100).toFixed(0)}%`) : chalk.green('✓'),
          ]),
        ];
      ...
      console.log(`  Current section: ${scoreData.progress?.currentSectionProgress ?? 0}/${scoreData.progress?.currentSectionTotal ?? 0}`);
...
function getScoreColor(score: number): chalk.Chalk {
  if (score >= 0.95) return chalk.green;
  if (score >= 0.7) return chalk.yellow;
  if (score >= 0.4) return chalk.hex('#FFA500'); // Orange
  return chalk.red;
}

function getScoreStatus(score: number): string {
  if (score >= 0.95) return chalk.green('✅ Ready for Production');
  if (score >= 0.7) return chalk.yellow('⚠️  Needs Improvement');
  if (score >= 0.4) return chalk.hex('#FFA500')('🔶 Significant Gaps');
