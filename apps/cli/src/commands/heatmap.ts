  ...
  .action(async (sessionId: string | undefined, options) => {
    ...
    try {
      ...
      if (options.stdout) {
        ...
        const outputPath = options.output || `quiz2biz-heatmap-${targetSessionId.substring(0, 8)}${extension}`;
...
function formatMarkdown(data: HeatmapData): string {
  ...
  data.dimensions.forEach(dim => {
    const status = dim.score >= 0.95 ? '🟢' : dim.score >= 0.7 ? '🟡' : dim.score >= 0.4 ? '🟠' : '🔴';
    lines.push(`| ${dim.name} | ${(dim.score * 100).toFixed(0)}% | ${dim.questionsAnswered}/${dim.totalQuestions} | ${status} |`);
  });
  ...
  data.dimensions.forEach(dim => {
    ...
    dim.questions.forEach(q => {
      ...
      const status = residual <= 0.05 ? '🟢' : residual <= 0.15 ? '🟡' : residual <= 0.30 ? '🟠' : '🔴';
      const questionText = q.text.length > 50 ? q.text.substring(0, 47) + '...' : q.text;
      lines.push(`| ${questionText} | ${(q.severity * 100).toFixed(0)}% | ${(q.coverage * 100).toFixed(0)}% | ${status} |`);
    });
...
function formatCSV(data: HeatmapData): string {
  ...
  data.dimensions.forEach(dim => {
    dim.questions.forEach(q => {
      const residual = q.severity * (1 - q.coverage);
      const status = residual <= 0.05 ? 'GREEN' : residual <= 0.15 ? 'YELLOW' : residual <= 0.30 ? 'ORANGE' : 'RED';
      const escapedText = `"${q.text.replace(/"/g, '""')}"`;
      rows.push(`${dim.name},${q.id},${escapedText},${q.severity.toFixed(2)},${q.coverage.toFixed(2)},${residual.toFixed(2)},${status}`);
    });
  });
