...
const getRiskColorClass = (residualRisk: number): string => {
    if (residualRisk <= 0.2) return 'bg-green-500';
    if (residualRisk <= 0.4) return 'bg-green-300';
    if (residualRisk <= 0.5) return 'bg-yellow-400';
    if (residualRisk <= 0.6) return 'bg-orange-400';
    if (residualRisk <= 0.8) return 'bg-red-400';
    return 'bg-red-600';
};
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-4">
                    <div className="text-right">
                        ...
                        <p className={`text-2xl font-bold ${overallScore >= 80 ? 'text-green-600' : overallScore >= 60 ? 'text-yellow-600' : 'text-red-600'}`}>
                            {overallScore}%
                        </p>
                    </div>
                </div>
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                Weight: {(selectedDimensionData.weight * 100).toFixed(1)}% |
                                Average Coverage: {(selectedDimensionData.averageCoverage * 100).toFixed(0)}%
                            </p>
                        </div>
                        <button
                        ...
                        {selectedDimensionData.cells.map((cell) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className={`flex items-center gap-4 ${getTextColorClass(cell.residualRisk)}`}>
                                        ...
                                        <span className="font-bold">
                                            {Math.round((1 - cell.residualRisk) * 100)}%
                                        </span>
                                    </div>
                                </button>
