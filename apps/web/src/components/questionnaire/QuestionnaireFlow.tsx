...
export const QuestionnaireFlow: React.FC<QuestionnaireFlowProps> = ({
    ...
    const validateCurrentQuestion = useCallback((): boolean => {
        ...
        if (question.isRequired && (!response || response.value === undefined || response.value === '')) {
            setErrors((prev) => {
                const next = new Map(prev);
    ...
    }, [currentIndex, totalQuestions, allQuestions, currentItem, validateCurrentQuestion, onSectionChange, onComplete]);
            ...
            <div className="mb-6">
                <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
                    <span className="px-2 py-1 bg-gray-100 rounded">
                        Section {sections.findIndex((s) => s.id === currentItem.section.id) + 1} / {sections.length}
                    </span>
                    <span>|</span>
