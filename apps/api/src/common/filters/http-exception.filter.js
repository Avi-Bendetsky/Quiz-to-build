...
var HttpExceptionFilter = function () {
    ...
    var HttpExceptionFilter = _classThis = /** @class */ (function () {
        ...
        HttpExceptionFilter_1.prototype.catch = function (exception, host) {
            ...
            // Log error details
            this.logger.error("".concat(request.method, " ").concat(request.url, " - ").concat(status, " - ").concat(message), exception instanceof Error ? exception.stack : undefined);
            response.status(status).json(errorResponse);
        };
        HttpExceptionFilter_1.prototype.getErrorCode = function (status) {
            var errorCodes = {
                400: 'BAD_REQUEST',
                401: 'UNAUTHORIZED',
                403: 'FORBIDDEN',
