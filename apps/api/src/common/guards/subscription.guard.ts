...
export class SubscriptionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const featureCheck = this.reflector.getAllAndOverride<FeatureCheckConfig>(
            FEATURE_CHECK_KEY,
            [context.getHandler(), context.getClass()],
        );
    ...
    private async checkFeatureAccess(
        ...
        const currentUsage = config.getUsage
            ? await config.getUsage(request)
            : 0;
...
export const FEATURE_TIER_MATRIX = {
    ...
    exportFormats: { FREE: ['PDF'], PROFESSIONAL: ['PDF', 'DOCX'], ENTERPRISE: ['PDF', 'DOCX', 'JSON', 'XML'] },
