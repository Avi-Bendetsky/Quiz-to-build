    ...
    private readonly templates: PolicyTemplate[] = [
        {
            ...
            statementTemplates: [
                { text: 'All systems shall implement defense-in-depth security controls', requirement: RequirementLevel.SHALL },
                { text: 'Security architecture shall be reviewed quarterly', requirement: RequirementLevel.SHALL },
                { text: 'All code should undergo security review before deployment', requirement: RequirementLevel.SHOULD },
            ],
            standardRequirements: [
                { description: 'Implement TLS 1.3 for all data in transit', specification: 'TLS 1.3+ required' },
                { description: 'Encrypt data at rest using AES-256', specification: 'AES-256-GCM encryption' },
        ...
        },
        {
            ...
            statementTemplates: [
                { text: 'All infrastructure shall be managed as code', requirement: RequirementLevel.SHALL },
                { text: 'Changes shall be deployed through automated pipelines', requirement: RequirementLevel.SHALL },
                { text: 'Production deployments should require approval', requirement: RequirementLevel.SHOULD },
            ],
            standardRequirements: [
                { description: 'Use Terraform/Bicep for infrastructure provisioning', specification: 'IaC required' },
                { description: 'CI/CD pipeline with security gates', specification: 'SAST/DAST required' },
                { description: 'Immutable infrastructure deployment', specification: 'Blue-green deployment' },
            ],
            procedureSteps: [
        ...
        },
        {
            ...
            statementTemplates: [
                { text: 'Personal data shall be processed lawfully and transparently', requirement: RequirementLevel.SHALL },
                { text: 'Data subject rights shall be facilitated within 30 days', requirement: RequirementLevel.SHALL },
                { text: 'Privacy impact assessments should be conducted for new processing', requirement: RequirementLevel.SHOULD },
            ],
            standardRequirements: [
                { description: 'Maintain data processing register', specification: 'GDPR Article 30 compliant' },
                { description: 'Implement consent management', specification: 'Granular, revocable consent' },
                { description: 'Enable data subject rights automation', specification: '< 30 day response' },
            ],
            procedureSteps: [
