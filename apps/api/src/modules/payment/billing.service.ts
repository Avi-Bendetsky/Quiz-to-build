...
export class BillingService {
    ...
    async recordPayment(params: RecordPaymentParams): Promise<void> {
        ...
        for (const org of orgs) {
            ...
            const billingHistory = (existingSettings.billingHistory as Array<Record<string, unknown>>) || [];
    ...
    async recordPaymentFailure(params: RecordPaymentFailureParams): Promise<void> {
        ...
        for (const org of orgs) {
            const existingSettings = (org.settings as Record<string, unknown>) || {};
            const paymentFailures = (existingSettings.paymentFailures as Array<Record<string, unknown>>) || [];
    ...
    async getBillingSummary(organizationId: string): Promise<{
        ...
        const lastFailure = settings.lastPaymentFailure as { failureReason: string; failedAt: string } | null;
