...
export class QpgService {
    ...
    async generatePromptsForSession(sessionId: string): Promise<PromptBatch> {
        ...
        for (const gap of gaps) {
            const template = await this.templateService.getTemplateForDimension(
                gap.dimensionKey,
            );
            if (template) {
        ...
        // Calculate totals
        const totalEffortHours = prompts.reduce(
            (sum, p) => sum + p.estimatedEffort,
            0,
        );
        ...
        this.logger.log(
            `Generated ${prompts.length} prompts, total effort: ${totalEffortHours}h`,
        );
