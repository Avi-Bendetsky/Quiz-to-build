...
export class PromptGeneratorService {
    ...
    async generate(
        ...
        const constraints = template.defaultConstraints.map((c) =>
            this.interpolate(c, context),
        );

        // Generate deliverables
        const deliverables = template.defaultDeliverables.map((d) =>
            this.interpolate(d, context),
        );
    ...
    private calculatePriority(residualRisk: number): number {
        if (residualRisk > 0.2) return 1; // Critical
        if (residualRisk > 0.15) return 2; // High
        if (residualRisk > 0.1) return 3; // Medium
        if (residualRisk > 0.05) return 4; // Low
        return 5; // Minimal
    }
    ...
    private generateTags(gap: GapContext, template: PromptTemplate): string[] {
        const tags: string[] = [
            gap.dimensionKey,
            template.evidenceType.toLowerCase(),
        ];

        // Add priority tag
        const priority = this.calculatePriority(gap.residualRisk);
        if (priority === 1) tags.push('critical');
        else if (priority === 2) tags.push('high-priority');

        // Add standard refs as tags
        for (const ref of gap.standardRefs.slice(0, 3)) {
            const cleanRef = ref.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase();
            if (cleanRef) tags.push(cleanRef);
        }
