...
export interface GitLabEvidenceResult {
  type: 'pipeline' | 'job' | 'merge_request' | 'release' | 'vulnerability' | 'test_report' | 'coverage';
  sourceId: string;
  sourceUrl: string;
...
export class GitLabAdapter {
  ...
  async fetchMergeRequests(
    config: GitLabConfig,
    options: { state?: 'opened' | 'closed' | 'merged' | 'all'; perPage?: number; page?: number } = {},
  ): Promise<GitLabEvidenceResult[]> {
  ...
  async fetchCoverageHistory(
    ...
    try {
      ...
      let endpoint = `/projects/${projectId}/jobs?scope=success&per_page=50`;

      const jobs = await this.makeRequest<GitLabJob[]>(config, endpoint);
      const coverageJobs = jobs.filter((j) => j.coverage !== null && j.ref === ref);

      if (coverageJobs.length === 0) return null;
      ...
      return {
        ...
        data: {
          ...
          averageCoverage: coverageData.reduce((sum, j) => sum + (j.coverage || 0), 0) / coverageData.length,
        },
