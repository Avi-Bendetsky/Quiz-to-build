...
export class JiraConfluenceAdapter {
  ...
  async fetchIssues(
    ...
    const {
      ...
      maxResults = 50,
      startAt = 0,
      fields = ['summary', 'description', 'issuetype', 'status', 'priority', 'assignee', 'reporter', 'created', 'updated', 'resolutiondate', 'labels', 'components', 'fixVersions', 'customfield_10020', 'customfield_10016', 'parent', 'subtasks', 'attachment', 'comment'],
    } = options;

    const searchJql = jql || (projectKey ? `project = ${projectKey} ORDER BY updated DESC` : 'ORDER BY updated DESC');
    
    const endpoint = `/search?jql=${encodeURIComponent(searchJql)}&maxResults=${maxResults}&startAt=${startAt}&fields=${fields.join(',')}`;

    const response = await this.makeRequest<{ issues: JiraIssue[]; total: number }>(config, endpoint);
    ...
    return response.issues.map((issue) => ({
      ...
      data: {
        ...
        fixVersions: issue.fields.fixVersions.map((v) => ({ name: v.name, releaseDate: v.releaseDate })),
  ...
  async searchPages(
    ...
    const response = await this.makeRequest<ConfluenceSearchResult>(config, endpoint, 'GET', undefined, true);
  ...
  async fetchPage(
    config: ConfluenceConfig,
    pageId: string,
    expand: string[] = ['space', 'body.storage', 'body.view', 'version', 'ancestors', 'children.page', 'children.attachment'],
  ): Promise<JiraEvidenceResult> {
  ...
  async ingestAllEvidence(
    ...
    this.logger.log(
      `Jira/Confluence ingestion complete: ${totalIngested} items`,
    );
