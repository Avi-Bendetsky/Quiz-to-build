...
export class GitHubAdapter {
    ...
    async fetchWorkflowRuns(
        ...
        const response = await this.makeRequest<{ workflow_runs: GitHubWorkflowRun[] }>(config, endpoint);
        ...
        return response.artifacts
            ...
            .map((artifact) => ({
                ...
                metadata: {
                    ...
                    isSbom: artifact.name.toLowerCase().includes('sbom') ||
    ...
    async fetchSecurityAdvisories(config: GitHubConfig): Promise<GitHubEvidenceResult[]> {
        try {
            ...
            const alerts = await this.makeRequest<Array<{
                number: number;
                state: string;
