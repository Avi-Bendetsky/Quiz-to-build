...
export class AdapterConfigService {
   ...
   */
  validateConfig(type: AdapterType, config: Record<string, unknown>): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    switch (type) {
      ...
      case 'gitlab':
        ...
        if (!config.projectId) errors.push('GitLab project ID is required');
        break;

      case 'jira':
        if (!config.domain) errors.push('Jira domain is required');
        if (!config.email) errors.push('Jira email is required');
        if (!config.apiToken) errors.push('Jira API token is required');
        if (!config.projectKey) errors.push('Jira project key is required');
        break;

      case 'confluence':
        if (!config.domain) errors.push('Confluence domain is required');
        if (!config.email) errors.push('Confluence email is required');
        if (!config.apiToken) errors.push('Confluence API token is required');
        if (!config.spaceKey) errors.push('Confluence space key is required');
        break;

      case 'azure_devops':
        if (!config.organizationUrl) errors.push('Azure DevOps organization URL is required');
        if (!config.personalAccessToken) errors.push('Azure DevOps PAT is required');
        if (!config.project) errors.push('Azure DevOps project is required');
        break;
    }
  ...
  getAdapterTypeInfo(type: AdapterType): {
    ...
    const info: Record<AdapterType, { displayName: string; description: string; icon: string; capabilities: string[] }> = {
      github: {
        displayName: 'GitHub',
