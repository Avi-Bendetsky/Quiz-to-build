...
export class TeamsWebhookController {
    ...
    @ApiOperation({
        ...
        description: 'Generate and send a readiness heatmap Adaptive Card to the configured Teams channel.',
    })
    @ApiBody({
        schema: {
            type: 'object',
            properties: {
                webhookUrl: { type: 'string', description: 'Teams webhook URL (optional, uses env if not provided)' },
    ...
    @ApiOperation({
        ...
        description: 'Generate and send an approval request Adaptive Card for two-person rule workflows.',
    })
    @ApiResponse({ status: 200, description: 'Card sent successfully' })
    ...
    async previewCard(
        ...
        switch (cardType) {
            ...
            case 'gaps':
                ...
                break;
            case 'approval':
                card = this.adaptiveCardService.generateApprovalRequestCard(sampleData as ApprovalRequestCardData);
                break;
            case 'score-update':
    ...
    private getSampleData(cardType: string): HeatmapCardData | GapSummaryCardData | ApprovalRequestCardData | ScoreUpdateCardData {
        switch (cardType) {
            case 'heatmap':
                return {
                    ...
                    overallScore: 0.72,
                    dimensions: [
                        { name: 'Architecture & Security', score: 0.85, questionsAnswered: 12, totalQuestions: 14 },
            ...
            case 'gaps':
                return {
                    ...
                    highGaps: 5,
                    gaps: [
                        { question: 'Is there a documented disaster recovery plan?', dimension: 'Security', severity: 0.9, coverage: 0.1, questionId: 'q1' },
                        { question: 'Are secrets stored in a vault?', dimension: 'Security', severity: 0.85, coverage: 0.2, questionId: 'q2' },
                        { question: 'Is there automated testing coverage?', dimension: 'Testing', severity: 0.7, coverage: 0.3, questionId: 'q3' },
                    ],
