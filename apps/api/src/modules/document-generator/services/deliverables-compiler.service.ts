...
export class DeliverablesCompilerService {
  ...
  async compileDeliverablesPack(
    ...
    documents.push(
      this.compileArchitectureDossier(session, responses, dimensions, autoSection, maxWordsPerSection),
    );
    ...
    documents.push(
      this.compileDevSecOpsDocument(session, responses, dimensions, autoSection, maxWordsPerSection),
    );
    ...
    documents.push(
      this.compileObservabilityDocument(session, responses, dimensions, autoSection, maxWordsPerSection),
    );
    ...
    if (includeDecisionLog && decisions.length > 0) {
      documents.push(
        this.compileDecisionLog(session, decisions, autoSection, maxWordsPerSection),
      );
    }
    ...
    if (includeReadinessReport) {
      documents.push(
        this.compileReadinessReport(session, dimensions, evidenceItems, autoSection, maxWordsPerSection),
      );
    }
  ...
  private compileArchitectureDossier(
    ...
    const archResponses = responses.filter(
      (r) => r.question.dimension?.key === 'arch_sec',
    );
  ...
  private compileSDLCPlaybook(
    ...
    const devResponses = responses.filter(
      (r) => r.question.dimension?.key === 'devops_iac' || r.question.dimension?.key === 'quality_test',
    );
  ...
  private compileTestStrategy(
    ...
    const testResponses = responses.filter(
      (r) => r.question.dimension?.key === 'quality_test',
    );
  ...
  private compilePrivacyDocument(
    ...
    const privacyResponses = responses.filter(
      (r) => r.question.dimension?.key === 'privacy_data',
    );
  ...
  private compileObservabilityDocument(
    ...
    const obsResponses = responses.filter(
      (r) => r.question.dimension?.key === 'observe_sre',
    );
  ...
  private compileFinanceDocument(
    ...
    const financeResponses = responses.filter(
      (r) => r.question.dimension?.key === 'finance_unit',
    );
  ...
  private compilePolicyDocument(
    ...
    const policyResponses = responses.filter(
      (r) => r.question.dimension?.key === 'governance_risk' || r.question.dimension?.key === 'policy_ctrl',
    );
...
Based on ${responses.length} architecture-related responses, this document outlines the system design principles and implementation guidelines.
  ...
  private generateArchitectureDecisions(responses: any[]): string {
    const decisions = responses.filter((r) => r.question.text.toLowerCase().includes('decision') || r.question.text.toLowerCase().includes('architecture'));
    ...
    if (decisions.length === 0) {
      content += 'No specific architecture decisions documented. Refer to the questionnaire responses for details.\n';
    }
