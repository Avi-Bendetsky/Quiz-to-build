...
describe('DocumentGeneratorService', () => {
  ...
  describe('regenerateDocument', () => {
    it('regenerates existing document', async () => {
      const mockDoc = {
        id: 'doc-123',
        sessionId: 'session-123',
        documentTypeId: 'doc-type-789',
      ...
      mockDocumentBuilder.buildDocument.mockResolvedValue(Buffer.from('new content'));
      mockStorage.upload.mockResolvedValue({
        url: 'https://storage.blob/documents/file-v2.docx',
        fileName: 'file-v2.docx',
        fileSize: 2048000,
      });

      const result = await service.regenerateDocument('doc-123', 'user-456');
