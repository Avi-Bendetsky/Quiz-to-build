...
export class DeliverablesCompilerController {
    constructor(
        private readonly compilerService: DeliverablesCompilerService,
    ) { }
    ...
    @ApiOperation({
        ...
        description: 'Generates all Quiz2Biz deliverables including Architecture Dossier, SDLC Playbook, Test Strategy, DevSecOps, Privacy, Observability, Finance documents, Policy pack, Decision Log, and Readiness Report.'
    })
    @ApiResponse({ status: 201, description: 'Deliverables pack compiled successfully', type: DeliverablePackResponseDto })
    ...
    async compileDeliverables(
        ...
        return this.compilerService.compileDeliverablesPack(
            dto.sessionId,
            user.id,
            {
    ...
    @ApiQuery({
        name: 'category',
        enum: ['ARCHITECTURE', 'SDLC', 'TESTING', 'DEVSECOPS', 'PRIVACY', 'OBSERVABILITY', 'FINANCE', 'GOVERNANCE', 'READINESS'],
    ...
    async getDocumentByCategory(
        ...
        const pack = await this.compilerService.compileDeliverablesPack(
            sessionId,
            user.id,
            { autoSection: true },
        );
    ...
    @ApiOperation({
        ...
        description: 'Retrieve summary information about the deliverables pack without full document content.'
    })
    ...
    async getDecisionLog(
        ...
        const pack = await this.compilerService.compileDeliverablesPack(
            sessionId,
            user.id,
            { includeDecisionLog: true, autoSection: true },
        );

        const decisionLog = pack.documents.find(
            (doc) => doc.title.toLowerCase().includes('decision log'),
        );
    ...
    async getReadinessReport(
        ...
        const pack = await this.compilerService.compileDeliverablesPack(
            sessionId,
            user.id,
            { includeReadinessReport: true, autoSection: true },
        );

        const readinessReport = pack.documents.find(
            (doc) => doc.category === 'READINESS',
        );
