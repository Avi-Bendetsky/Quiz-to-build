...
describe('HeatmapService', () => {
    ...
    describe('getCells', () => {
        ...
        it('filters cells by both dimension and severity', async () => {
            ...
            expect(cells.every((c) => c.dimensionKey === 'security' && c.severityBucket === 'High')).toBe(true);
        });
    });
    ...
    describe('drilldown', () => {
        ...
        it('throws NotFoundException for non-existent cell', async () => {
            ...
            await expect(service.drilldown('session-123', 'non-existent', 'High')).rejects.toThrow(NotFoundException);
        });
    });
