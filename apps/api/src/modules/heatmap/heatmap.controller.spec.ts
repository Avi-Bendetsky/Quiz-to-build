...
- **R Red**: Residual > 0.15 (high risk)`;
    ...
    describe('getCells', () => {
        ...
        it('filters cells by dimension', async () => {
            ...
            expect(mockHeatmapService.getCells).toHaveBeenCalledWith('session-123', 'security', undefined);
        });
        ...
        it('filters cells by both dimension and severity', async () => {
            ...
            const result = await controller.getCells('session-123', { dimension: 'security', severity: 'Critical' });

            expect(result.length).toBe(1);
            expect(result[0].dimensionKey).toBe('security');
            expect(result[0].severityBucket).toBe('Critical');
            expect(mockHeatmapService.getCells).toHaveBeenCalledWith('session-123', 'security', 'Critical');
        });
    });
            ...
            const mockHeatmap = {
                ...
                cells: [
                    ...
                    },
                    {
                        dimensionKey: 'security',
                        severityBucket: 'Medium',
                        cellValue: 0.10, // Should be AMBER (0.05-0.15)
                        colorCode: HeatmapColor.AMBER,
                        questionCount: 1,
                    },
                    {
