...
export const SeverityBuckets = {
    getBucket(severity: number): SeverityBucket {
        if (severity < 0.25) return SeverityBucket.LOW;
        if (severity < 0.50) return SeverityBucket.MEDIUM;
        if (severity < 0.75) return SeverityBucket.HIGH;
...
export const HeatmapColors = {
    getColor(residual: number): HeatmapColor {
        if (residual <= 0.05) return HeatmapColor.GREEN;
        if (residual <= 0.15) return HeatmapColor.AMBER;
