    ...
    async updateDecisionStatus(
        ...
        if (dto.status !== DecisionStatus.LOCKED) {
            throw new BadRequestException(
                `Invalid status transition. Only DRAFT -> LOCKED is allowed.`
            );
        }
        ...
        this.logger.log(
            `Decision locked: ${dto.decisionId} by ${userId}`,
        );
    ...
    async supersedeDecision(
        ...
        if (!originalDecision) {
            throw new NotFoundException(
                `Original decision not found: ${dto.supersedesDecisionId}`
            );
        }
